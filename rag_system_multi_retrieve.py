...
class RAGSystem:
    ...
    def answer_question(self, question: str, top_k: int = 3) -> Dict:
        """Answer a question using multi-retriever without reranker in the RAG system"""
        ...
        merged_results = sorted(merged_results, key=lambda x: x[1], reverse=True)[:k]  # 直接取 top-k，不做 rerank

        # 🔹 Step 3: Generate answer
        answer = self.generator.generate_answer(question, merged_results, self.config.max_length)
        ...
        return {
            ...
            'answer': answer,
            'sources': sources,
            'num_retrieved': len(merged_results),
            'config': {
...
def main():
    ...
    evaluation_results = rag.evaluate_system(
        ...
        top_k=3,
        save_results_file="eval/evaluation_results_multi_retrieve.json",
        run_ablation=True
    )
